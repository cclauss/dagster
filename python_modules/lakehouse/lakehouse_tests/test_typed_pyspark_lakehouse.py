...
def typed_pyspark_table(spark_type, name=None, input_defs=None, description=None):
    def _wrap(fn):
...
def test_spark_type_from_kwargs():
    # jsonValue to avoid py3.5 ordering issues
    assert (
        spark_type_from_kwargs(id=int, number=int).jsonValue()
